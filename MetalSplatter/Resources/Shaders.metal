...
using triangle_mesh_t = metal::mesh<ColorInOut, void, 256, 384, metal::topology::triangle>;
static constant half2 relativeCoordinatesArray[] = { { -1, -1 }, { -1, 1 }, { 1, -1 }, { 1, 1 } };
...
void splatMeshShader(triangle_mesh_t output_mesh,
    ...
    uint end = min((uint)64, total - gid * 64);
    uint splat_count = 0;
    for (int i=0; i<end; i++) {
        Splat splat = splatArray[gid * 64 + i];
        ...
        output_mesh.set_index(splat_count*6+0, splat_count*4+0);
        output_mesh.set_index(splat_count*6+1, splat_count*4+1);
        output_mesh.set_index(splat_count*6+2, splat_count*4+2);
        output_mesh.set_index(splat_count*6+3, splat_count*4+1);
        output_mesh.set_index(splat_count*6+4, splat_count*4+2);
        output_mesh.set_index(splat_count*6+5, splat_count*4+3);
        ...
        for (int j=0; j<4; j++) {
            ...
            * kBoundsRadius
            ...
            output_mesh.set_vertex(splat_count*4+j, fragmentInput);
        }

        splat_count += 1;
    }

    output_mesh.set_primitive_count(splat_count * 6);
}
